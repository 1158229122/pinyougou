...
public class SeckillOrderController {
	...
	public TbSeckillGoods findOneFromRedis(Long SeckillGoodsId){
	 	return 	seckillOrderService.findOneFromRedis(SeckillGoodsId);
	}
	...
	public Result submitCreateOrder(Long id){
		String userId = SecurityContextHolder.getContext().getAuthentication().getName();
		if("anonymousUser".equals(userId)){//如果未登录
			return new Result(false, "您未登录");
		}
		try {
			seckillOrderService.submitCreateOrder(id,userId);
			return new Result(true,"抢购成功");
		} catch (RuntimeException e){
			return new Result(false,e.getMessage());
		} catch (Exception e) {
			e.printStackTrace();
			return new Result(false,"手慢了!抢购失败");
		}
